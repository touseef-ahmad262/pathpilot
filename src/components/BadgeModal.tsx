...
export const BadgeModal: React.FC<BadgeModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/80 backdrop-blur-md overflow-y-auto">
      <div className="relative w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden my-4 sm:my-8">
        {/* Header */}
        ...
        {/* Badges List */}
        <div className="p-4 sm:p-6 space-y-2.5 sm:space-y-3 max-h-[60vh] overflow-y-auto">
          {gamification.badges.map((badge) => (
            <div
              key={badge.id}
              className={`p-3 sm:p-4 rounded-xl border transition-all flex items-start gap-2.5 sm:gap-3.5 ${
                badge.unlocked
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  ...
                  {badge.unlocked ? (
                    <span className="text-[10px] font-bold uppercase tracking-wider text-amber-300 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20 flex items-center gap-1 shrink-0">
